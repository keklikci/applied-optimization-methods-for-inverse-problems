...
def test_gradient_descent_opt(
    output_path: str, learning_rate: float = 0.001, n_iterations: int = 100
) -> None:
...
def test_tikhonov_opt(
    output_path: str, learning_rate: float = 0.0005, n_iterations: int = 100_000
) -> None:
    volume_shape = [128, 128]
    sinogram_shape = [128]
    d2c = volume_shape[0] * 100.0
    ...
    os.makedirs(output_path, exist_ok=True)
    plt.savefig(
        os.path.join(output_path, "tikhonov_regularization.png"), transparent=True
    )


def test_huber_functional_opt(
    output_path: str,
...
def test_fair_potential_opt(
    ...
    for i in range(n_iterations):
        if i % 100_000 == 0:
            if i == 0:
                print("Optimizing via Fair potential...")
            else:
                print(f"Processed {i} iterations.")
        # add term to the gradient
        gradient = operator.applyAdjoint(operator.apply(x) - sinogram) + (
